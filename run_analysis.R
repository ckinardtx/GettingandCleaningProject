...
# Now assign row number as the values of ID column
X_train$ID <- as.numeric(rownames(X_train))
# Next read activity training data
y_train = read.table("UCI HAR Dataset/train/y_train.txt", col.names=c("activity_id"))  # max = 6
# This y_train is now equal to merge(y_train, activity_labels)
# Assign row number as the values of ID column
y_train$ID <- as.numeric(rownames(y_train))
# Merge subject_train and y_train into train
train <- merge(subject_train, y_train, all=TRUE)
# Now merge train and X_train
...
# Remember y_test = merge(y_test, activity_labels)
# Assign row number as the values of ID column
...
# Extract selected measurements on the mean and standard deviation for each measurement.
